...
class DiagCovOnlyMoreProjection(object):
    ...
    def more_step(self, eps_sigma, old_var, target_var):
        self._eps_sigma = eps_sigma
        self._succ = True

        self._old_precision = 1.0 / old_var

        self._old_chol_precision = np.sqrt(self._old_precision)

        self._target_precision = 1.0 / target_var

        self._old_logdet = - 2 * np.sum(np.log(self._old_chol_precision + 1e-25))
        self._kl_const_part = self._old_logdet - self._dim

        # try:
        opt_eta_sig = self.opt_dual(self._dual_var)
        self._eta_sig = opt_eta_sig

        self._proj_var = self._new_var(opt_eta_sig)
        self._proj_precision = (opt_eta_sig * self._old_precision + self._target_precision) / (opt_eta_sig + 1)
            
        # except Exception:
        #     self._succ = False
        #     self._proj_var = None

        return self._proj_var


    def _new_var(self, eta_sig):
        return 1.0 / ((eta_sig * self._old_precision + self._target_precision) / (eta_sig + 1))

    def _dual_var(self, eta_sig, grad):
        eta_sig = eta_sig[0] if eta_sig[0] > 0.0 else 0.0
        self._lp = eta_sig
        try:
            proj_var = self._new_var(eta_sig)
            proj_cov_chol = np.sqrt(proj_var)
            new_logdet = 2 * np.sum(np.log(proj_cov_chol + 1e-25))

            dual = eta_sig * self._eps_sigma - 0.5 * eta_sig * self._old_logdet
            dual += 0.5 * (eta_sig + 1) * new_logdet

            trace_term = np.sum(np.square(self._old_chol_precision % proj_cov_chol))
